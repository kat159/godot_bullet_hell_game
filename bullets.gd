...
func _ready():
	...
	for _i in BULLET_COUNT:
		...
		PhysicsServer2D.body_set_collision_mask(bullet.body, 2) # **因为不设置的话默认为1， 为0表示不处理，相当于null
		PhysicsServer2D.body_set_collision_layer(bullet.body, 1) # 设置layer，可以不用，因为默认为1
		
		# make bullet push player when collide
		# PhysicsServer2D.body_set_mode(bullet.body, PhysicsServer2D.BODY_MODE_KINEMATIC)
		# PhysicsServer2D.body_set_constant_force(bullet.body, Vector2(100, 0))
